...
| **Kubernetes** | A ServiceAccount with `get`/`list`/`watch` on sixteen kinds, plus `get`/`list` on namespaces and `list` on secrets to read Helm's release records. [Setup](https://cloudkeel.io/docs/integrations/kubernetes/) | n/a: Helm's own records are the desired state |
